...
- 256-bit 随机令牌和 Helper 绝对路径只保存 SHA-256 哈希；
...
已知限制：本地 MCP 不等于第三方 AI 客户端离线；用户移动或改名 App 会使客户端中的 Helper 绝对路径失效。无线麦SayAll.app 会依据私有路径指纹提示先移除再重新连接，不会自动覆盖其他 MCP 配置。
