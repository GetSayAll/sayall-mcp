...
- 版本：`0.1.0`
- 平台：macOS
- 当前状态：候选实现，等待真实 Codex MCP 验收

## 测试前准备

1. 安装 Node.js 22 或以上版本，在仓库根目录执行 `npm install && npm run build`。
2. 安装已经支持本地语音记录的 Remote Mic，并在“语音记录”页开启保存功能。
3. 在 TextEdit 和 Codex 中分别产生至少两条语音记录，确认 Remote Mic 页面中能够看到。
...
1. 在没有执行 `remote-mic enable` 的新环境中执行 `remote-mic authorize --name TestAgent`。
2. 尝试以任意 client ID 和令牌启动 `remote-mic serve`。
...
预期结果：返回 Remote Mic 历史中的 App 名称、Bundle ID、记录数和最早/最新时间；不返回任何转写正文。
...
1. 把配置中的令牌改错一个字符后连接。
2. 恢复正确配置，确认可以读取。
3. 执行 `remote-mic revoke --client-id <UUID>`。
4. 不重启 Codex，再次调用两个工具。
...
1. 新建一个有效客户端并成功读取。
2. 执行 `remote-mic disable`。
3. 再次调用 MCP。
4. 打开 Remote Mic“语音记录”页面。

预期结果：所有客户端立即被拒绝；Remote Mic 中已有历史保持不变；重新开启后未撤销的令牌恢复可用。
...
2. 同时保留一个合法日期文件。
3. 查询时在 Remote Mic 页面删除另一条测试记录。

预期结果：损坏文件计入 `skippedFileCount`，合法记录仍可读；并发删除不会崩溃、恢复或改写历史。

失败判定：整个查询失败、返回损坏内容、恢复已删除记录或修改 Remote Mic 文件。
...
失败判定：出现 localhost 或局域网监听、访问 Remote Mic Web Relay、退出后仍常驻。

## 稳定功能回归

- Remote Mic 继续按 App 和日期保存、显示、复制和可恢复删除历史；
...
自动化覆盖历史解析、Apple 日期转换、App 汇总、过滤、分页、畸形 cursor、损坏文件、符号链接拒绝、默认关闭、令牌哈希、文件权限、错误令牌、撤销、关闭和 MCP 工具级撤销复查。类型检查和构建只证明源码边界。

代理已使用构建后的只读解析器加载本机真实 Remote Mic 历史，确认实际 JSON Schema、Codex/TextEdit 分组和 Apple 日期转换兼容，过程中没有输出正文。当前尚未把真实用户令牌写入 Codex 配置，也未使用真实历史正文执行云端 Agent 调用，因此不能表述为已经完成真实 Codex 数据流验收。用户需要确认其选定 AI 客户端是否会把 MCP 返回内容上传给第三方服务商。
